...
def area(room):
    return float(room[4] * room[5])

def find_room(floor, rid):
    return next((r for r in ROOMS[floor] if r[0] == ridid), None)
...
def make_floor_heatmap(floor, weights, selected_floor, selected_room_id, design):
    ...
    for room in ROOMS[floor]:
        rid,name,x,y,w,h,kind=room; cx,cy=room_center(room); p=weights[(floor,rid)]
    ...
    for rid,name,x,y,w,h,kind in ROOMS[floor]:
        ...
        cx,cy=room_center((rid,name,x,y,w,h,kind))
